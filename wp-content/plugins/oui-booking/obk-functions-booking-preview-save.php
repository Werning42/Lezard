...
if ($ok){
	...
	if (!$forEmail){
		$previsualisation .= '<form action="" method="POST" id="obk_formAllDataBooking">';	
		foreach($SAFE_DATA as $key => $value){
			if ($key != 'obk_act'){
				$previsualisation .= '<input type="hidden" name="'.$key.'" value="'.obk_removeslashes($value).'">';
			}
		}						
		$previsualisation .= '<input type="submit" value="'.__('Edit my entry', 'oui-booking').'"></form>';
	
		$previsualisation .= '
		<form action="" name="obk_saveBooking" method="post" onsubmit="return false">
	...
	$previsualisation .= '<script>obk_saveBooking()</script></div><div id="obk_calendarLoading"><div class="obk_loadingAnimation"></div></div></div>';
}